...
要求：
1. 语言必须极其专业且“口语化/业务化”（模拟业务人员或高管向你汇报的真实话术，绝不要生成刻板的、像监控工具吐出来的“系统警报/系统日志”格式）。
2. 直击该行业的经营痛点与管理词汇（如数据防出境安全、模型权限漂移、三方API风险等）。
3. 直接输出生成的业务文本内容，严禁包含任何 Markdown 格式包裹（不要用 \`\`\` 格式）、不要包含任何“好的，以下是为您生成的...”等前导或后置客套话。
4. 文本字数严格控制在 80 到 200 字之间。`;
